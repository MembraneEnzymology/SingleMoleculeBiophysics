...
/**
 * The step fitter plugin tries to find plateaus in step-like data. The
 * algorithm for finding steps starts by assuming only one step. It then tries
 * to split this step into two steps. The point at which to split a step is
 * determined by looking at the sum of squares. The challenge is to stop adding
 * steps when we risk to over-fit the data. This is done by looking at the
 * ration between the squared error of all the found-steps and the square error
 * of all the counter-steps.
 * 
 * Kerssemakers, Jacob WJ, et al. "Assembly dynamics of microtubules at molecular resolution." Nature 442.7103 (2006): 709-712.
 *
