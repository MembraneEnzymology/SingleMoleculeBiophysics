...
/**
 * This plugins colocalizes peaks on one side (either left or right) of the
 * image to peaks on the opposite site of the image. This plugin can be used
 * when you want to determine the translation between peaks on the left channel
 * compared to peaks on the right channel (or right to left) . Before running
 * this plugin it is necessary to run the peak finder plugin to determine all
 * the peaks that need to be colocalized.
 * 
 * Change history
...
public class Colocalization implements PlugIn {
	...
	public static double[] align(ImageProcessor ip, PointRoi proi) {
		...
		for (int i = 0; i < polygon.npoints; i++) {
			...
			double rSquared = PeakFitter.fitPeak(ip, p, e);
			
			xpoints[i] = p[2];
			ypoints[i] = p[3];
			
			PeakFitter.addToResultsTable(table, p, e, ip.getSliceNumber(), rSquared);
		}
		...
		for (int i = 0; i < xpoints.length; i++) {
			...
			// TODO add maximum distance parameter (currently fixed at 50 pixels)
			if (mindistance < 50) {
				dxs[npoints] = mindx;
				dys[npoints] = mindy;
				npoints++;
