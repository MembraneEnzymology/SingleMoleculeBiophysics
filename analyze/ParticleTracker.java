...
/**
 * This plugin tracks peaks in a movie. The plugin requires  a table with
 * localized peaks which can be obtained by the peak fitter plugin. For each
 * peak the plugin checks the next frame to see if there is a peak that is,
 * with some margin, the same position. If there is such a peak the two peaks
 * are linked and form a trajectory. The user can specify the maximum distance,
 * in pixels, a particle could have moved between two frames.
 * 
 * It should be noted that this plugin only works when there are not too many
 * particles. If there is uncertainty about which peak belong to which peak
