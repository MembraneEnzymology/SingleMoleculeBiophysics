...
/**
 * The PALM (Photo Activated Localization Microscopy) reconstructor plugin
 * creates a high-resolution image out of localized peak data. This plugin
 * requires a table with localized peaks which can be obtained by running the
 * peak fitter plugin.
 * 
 * Each localized peak is drawn as a 2 dimensional Gaussian in which the volume
 * is 1 and the width is determined by the localization precision of the fit
 * (the fitting error of the x and y parameters). By doing so the peaks with
 * the lowest fitting error will be more clearly visible compared to peaks with
 * a large fitting error.
 * 
...
public class PalmReconstructor implements PlugIn {
	...
	public void run(String arg0) {
		...
		dialog.addNumericField("pixel_size (nm)", 1, 2);
		...
		c.pixelWidth *= pixelSize;
		c.pixelHeight *= pixelSize;
		c.setUnit("nm");
		imp.show();
		IJ.run("Red Hot");
	}
